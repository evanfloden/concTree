...
use Cwd;
my $CWD_DIR = getcwd;
my $tmpdir="$CWD_DIR/tmp/$$";  # directory where all temporary files are written: /tmp/UID/PID
...
##############################################################################################
	...
	if ($a3mdir) {
	    ...
	    $command = "$hhbin/hhmake -i \$file -o $tmpdir/\$base.hhm";  #Remove Hash Here 1>/dev/null 2>>$logfile";
	    &HHPaths::System("$hhscripts/multithread.pl '".$a3mdir."' '$command' -cpu $cpu");	
